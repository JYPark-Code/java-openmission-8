...
public class AnswerController {
    ...
    public String answerVote(
        ...
        // 댓글 창을 열어둘 answerId를 그대로 쿼리 파라미터로 전달
        if (openAnswerId != null) {
            return String.format("redirect:/question/detail/%s?openAnswerId=%s#answer_%s",
                    answer.getQuestion().getId(), openAnswerId, answer.getId());
        } else {
            return String.format("redirect:/question/detail/%s#answer_%s",
                    answer.getQuestion().getId(), answer.getId());
        }
